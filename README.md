<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>PyLot - Miniature Autonomous Car</title>
</head>
<body>
    <h1>PyLot - Miniature Autonomous Car</h1>
    <p>This project focuses on developing a miniature autonomous car system using image processing and machine learning techniques.</p>

    <h2>Main Features</h2>
    <ul>
        <li><strong>Road Edge Detection</strong>:
            Applied Canny filtering and Hough Transform to detect road edges and generate a steering line for autonomous navigation.
        </li>
        <li><strong>Road Sign Detection</strong>:
            Built a road sign recognition system with <strong>99% accuracy</strong> using <strong>TensorFlow</strong> and <strong>OpenCV</strong>.
        </li>
    </ul>

    <h2>Project Structure</h2>
    <ul>
        <li><strong>1-frame-processing</strong>: Image frame processing for feature extraction.</li>
        <li><strong>2-video-processing</strong>: Video sequence processing for dynamic detection.</li>
        <li><strong>3-data-learning</strong>: Supervised learning from collected datasets.</li>
        <li><strong>4-deep-learning</strong>: Implementation of deep learning models for improved detection and navigation.</li>
        <li><strong>5-transfer-learning</strong>: Application of transfer learning to boost model performance on limited data.</li>
    </ul>
</html>
